...
// services/krc20.rs — KRC-20 token format detection
...
pub fn detect_krc20(tx: &offline_signer::transaction::model::Transaction) -> Krc20Info {
...
fn contains_substr_ci(data: &[u8], len: usize, needle: &[u8]) -> bool {
    ...
    for i in 0..=len - needle.len() {
        ...
        for j in 0..needle.len() {
            if !data[i + j].eq_ignore_ascii_case(&needle[j]) {
                matched = false;
                break;
            }
        }
