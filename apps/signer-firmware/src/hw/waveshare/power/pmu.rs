...
// hw/power/pmu_ws.rs — Backlight PWM control for Waveshare ESP32-S3-Touch-LCD-2
//
...
pub fn set_brightness_value(brightness: u8) {
    unsafe {
...
macro_rules! set_brightness {
    ($i2c:expr, $brightness:expr) => {
        $crate::hw::pmu::set_brightness_value($brightness)
    };
}
