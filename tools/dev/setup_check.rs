...
fn check_os() {
    ...
    if os == "windows" {
        ok("Windows: native PowerShell toolchain (WSL is not required).");
    }
}
...
fn check_build_env() {
    ...
    // Check if we're in the project root, apps/signer-firmware/, or tools/
    let at_root = std::path::Path::new("apps/signer-firmware/Cargo.toml").exists()
        && std::path::Path::new("tools/Cargo.toml").exists();
    let at_bootloader = std::fs::read_to_string("Cargo.toml")
        .map(|s| s.contains("kassigner-firmware"))
        .unwrap_or(false);
    let at_tools = std::fs::read_to_string("Cargo.toml")
        .map(|s| s.contains("firmware-tools"))
        .unwrap_or(false);
    let parent_is_root = std::path::Path::new("../apps/signer-firmware/Cargo.toml").exists()
        && std::path::Path::new("../tools/Cargo.toml").exists();
    ...
    if at_root {
        ...
        ok("Inside apps/signer-firmware/ — project root is ../");
...
fn main() {
    ...
    if issues == 0 {
        ...
        println!("    {CYAN}cd ../apps/signer-firmware{RESET}");
