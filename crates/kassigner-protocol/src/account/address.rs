...
pub fn decode_address(addr: &str) -> Result<(u8, [u8; 32]), String> {
    let prefix = address_prefix(addr)?;
    let data_part = &addr[prefix.len() + 1..];
    let values5 = decode_address_chars(data_part)?;
    let (payload5, checksum5) = values5.split_at(values5.len() - 8);
    verify_address_checksum(prefix, payload5, checksum5)?;
    decode_address_payload(payload5)
}

fn address_prefix(addr: &str) -> Result<&'static str, String> {
    ["kaspa", "kaspatest", "kaspasim", "kaspadev"]
        .into_iter()
        .find(|prefix| addr.starts_with(&format!("{prefix}:")))
        .ok_or_else(|| "Unknown address prefix".into())
}

fn decode_address_chars(data_part: &str) -> Result<Vec<u8>, String> {
    if data_part.len() < 9 {
        return Err("Address too short".into());
    }
    data_part.bytes().map(decode_address_char).collect()
}

fn decode_address_char(byte: u8) -> Result<u8, String> {
    let value = REV_CHARSET.get(usize::from(byte)).copied().unwrap_or(100);
    if value == 100 {
        Err(format!("Invalid character: '{}'", byte as char))
    } else {
        Ok(value)
    }
}

fn verify_address_checksum(prefix: &str, payload5: &[u8], checksum5: &[u8]) -> Result<(), String> {
    let fivebit_prefix = prefix.as_bytes().iter().map(|&c| c & 0x1f);
    let computed = checksum_value(payload5, fivebit_prefix);
    let expected = decoded_checksum(checksum5);
...
fn decoded_checksum(checksum5: &[u8]) -> u64 {
    let cs8 = conv5to8(checksum5);
    let mut buf = [0u8; 8];
    let copied = cs8.len().min(5);
    buf[3..3 + copied].copy_from_slice(&cs8[..copied]);
    u64::from_be_bytes(buf)
}

fn decode_address_payload(payload5: &[u8]) -> Result<(u8, [u8; 32]), String> {
...
fn conv8to5(payload: &[u8]) -> Vec<u8> {
    let padding = if payload.len().is_multiple_of(5) {
        0
    } else {
        1
    };
