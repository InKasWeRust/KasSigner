...
struct FixedLru {
    // Keep the fixed logical capacity while placing the backing arrays on the
    // allocator rather than in PreparedImage's stack-resident value. This no_std
    // fork already requires alloc for image pixels; heap-backing the LRU also
    // makes the RQRR_HEAP_BACKED contract true for its largest scratch object.
    keys: Box<[u8]>,
    values: Box<[ColoredRegion]>,
    ages: Box<[u32]>,
    len: usize,
    tick: u32,
}
...
impl FixedLru {
    fn new() -> Self {
        FixedLru {
            keys: alloc::vec![0; LRU_CAPACITY].into_boxed_slice(),
            values: alloc::vec![ColoredRegion::Tmp1; LRU_CAPACITY].into_boxed_slice(),
            ages: alloc::vec![0; LRU_CAPACITY].into_boxed_slice(),
            len: 0,
            tick: 0,
        }
    }
