//! Find and read QR-Codes — no_std fork for KasSigner
//!
//! Ported from rqrr 0.10.1 by WanzenBug.
//! Stripped std, replaced lru crate with inline fixed-size LRU,
//! replaced std::io::Write with minimal trait.
//!
//! Requires `alloc` (Vec, Box, String).

#![no_std]

// This no_std fork intentionally preserves the upstream arithmetic and minimal
// error surface used by the embedded signer. GF(2^n) addition is XOR, the
// unit error keeps the Write shim allocation-free, and the explicit modulo/
// ceiling arithmetic remains compatible with the older ESP toolchain.
...
/// Compile-time marker used by the firmware second-core worker.
...
/// Minimal Write trait replacing std::io::Write for no_std
pub trait Write {
    ...
    G: BitGrid,
{
    ...
    pub fn decode(&self) -> DeQRResult<(MetaData, String)> {
        ...
        let out = String::from_utf8(out).map_err(|_| DeQRError::EncodingError)?;
        Ok((meta, out))
    }
...
impl SimpleGrid {
    ...
    {
        let mut cell_bitmap = alloc::vec![0; (size * size + 7) / 8];
        let mut c = 0;
        for y in 0..size {
            for x in 0..size {
                if fill_func(x, y) {
